...
class AIPlayer:
    ...
    def minimax_search(self, state, depth_limit, alpha=-math.inf, beta=math.inf, maximizing_player=True):
    # Always try to mirror first when it's our turn
        if maximizing_player:
            mirror_move = self.get_mirror_move(state)
            if mirror_move:
                return math.inf, mirror_move  # Highest priority to mirror moves
    
    # Rest of minimax implementation remains the same...
        game_copy = ChainReactionGame(state.rows, state.cols)
        game_copy.set_state(*state.get_state_copy())
    ...
    def heuristic_explosion_potential(self, game_state):
        ...
        return score / (game_state.rows * game_state.cols)
    def get_mirror_move(self, game_state):
        """Returns a move that strictly mirrors the opponent's last move"""
        opponent = 'R' if self.player == 'B' else 'B'
        last_move = None
    
    # Find the opponent's most recent move (cell with count=1)
        for i in range(game_state.rows):
            for j in range(game_state.cols):
                cell = game_state.board[i][j]
                if cell['color'] == opponent and cell['count'] == 1:
                    last_move = (i, j)
                    break
            if last_move:
                break
        ...
        if last_move:
        # Calculate perfect mirror position
            row, col = last_move
            mirror_row = game_state.rows - 1 - row
            mirror_col = game_state.cols - 1 - col
        
        # Only return if the mirror position is completely empty
            if (game_state.board[mirror_row][mirror_col]['count'] == 0 and 
                game_state.is_valid_move(mirror_row, mirror_col, self.player)):
...
class GameUI:
    ...
    def play_human_vs_ai(self):
    # setting first player to Human (Red)
        self.game.current_player = 'R'
        self.save_state_to_file("Human")
